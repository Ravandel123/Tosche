...
async function startFishing(message, thread) {
   ...
   fishingCollector.on('collect', async i => {
      if (i.customId === 'start') {
         ...
         fishCaught = true;
         await i.update({ content: `Congratulations <@!${message.author.id}>! You caught **${C.strAddArticle(fish.name)}**! It weighs **${fish.data.weight} kg (${C.calcKgToImperial(fish.data.weight)})**!`, components: [] });
         fishingCollector.stop();
      }
   });
