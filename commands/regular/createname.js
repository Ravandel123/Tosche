...
module.exports = {
   ...
   usage: '[gender] [syllable] [position of the given syllable (1-4)] [amount of the syllables in generated names (max 4)] [amount of the generated names (max 100)]',
   example: 'male Clo 1 2 15',
   execute(message, args) {
      let response = '';
      const namesAmount = C.checkIfIntInRange(args[5], 1, 100) ? args[5] : 1;

      //Add function for multiple names creation and validate arguments inside
      for (let i = 0; i < namesAmount; i++) {
         response += NAMEGEN.createRandomName(args[1], args[2], args[3], args[4]);
